...
var contentMeta_default = '.content-meta {\n  margin-top: 0;\n  color: var(--darkgray);\n}\n.content-meta .content-meta-edit {\n  color: var(--secondary);\n  white-space: nowrap;\n}\n.content-meta[show-comma=true] > *:not(:last-child) {\n  margin-right: 8px;\n}\n.content-meta[show-comma=true] > *:not(:last-child)::after {\n  content: ",";\n}';

// src/components/ContentMeta.tsx
var defaultOptions = {
  showReadingTime: true,
  showComma: true,
  editRepository: void 0,
  editBranch: void 0,
  editPathPrefix: "content"
};
function encodeEditPath(path) {
  return path.split("/").filter((segment) => segment.length > 0).map(encodeURIComponent).join("/");
}
  ...
  function ContentMetadata({ cfg, fileData, displayClass }) {
    ...
    if (text) {
      ...
      if (options.editRepository && options.editBranch && fileData.relativePath) {
        const editPath = encodeEditPath([options.editPathPrefix, fileData.relativePath].filter(Boolean).join("/"));
        const editUrl = `${options.editRepository.replace(/\/$/, "")}/edit/${encodeURIComponent(options.editBranch)}/${editPath}`;
        segments.push(/* @__PURE__ */ u2("a", { class: "content-meta-edit", href: editUrl, target: "_blank", rel: "noopener noreferrer", children: "Edit note" }));
      }
