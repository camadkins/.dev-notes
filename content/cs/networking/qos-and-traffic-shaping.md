...
In packet-switched networks, quality of service refers to traffic prioritization and resource reservation control mechanisms rather than the achieved service quality. It is the ability to give different priorities to different flows, or to guarantee a bit rate, delay, delay variation, or loss rate to a flow. This matters most for real-time streaming multimedia such as voice over IP, multiplayer games, and IPTV, because those require a fairly fixed bit rate and are delay sensitive in a way that bulk transfers are not.
