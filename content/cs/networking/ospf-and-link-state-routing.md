...
Link-state routing protocols are one of the two main classes of routing protocol for packet-switched networks, the other being distance-vector. The basic concept is that every node constructs a map of the connectivity of the network as a graph showing which nodes connect to which, then independently calculates the best next hop to every possible destination, and that collection of best next hops forms its routing table.
