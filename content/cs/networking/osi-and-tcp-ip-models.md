...
Two machines can only talk if they agree on the rules at every level, from the voltage on the wire to the meaning of a web request. Rather than write one giant rulebook, network designers split the rules into layers, where each layer solves one problem and treats the layer below it as a service it does not have to understand. Two layering schemes get taught side by side. One is a formal reference model that names seven layers. The other is the four-layer family of protocols that carries real traffic.
...
The suite's protocols were built, deployed, and iterated by the Internet Engineering Task Force, and they were carrying traffic before the OSI model's own protocol stack was finished. Reality shipped first. The internet standardized on TCP/IP, and OSI became the map people use to describe it.
