...
The `(literal-id ...)` list in `syntax-rules` is easy to skim past. It changes what an identifier in a pattern means. Ordinarily an `id` in a pattern matches any syntax object, provided it is not bound to `...` or `_` and does not have the same binding as any `literal-id`, and it becomes a pattern variable for the clause. An `id` that *does* have the same binding as a `literal-id` instead matches only a syntax object that is an identifier with the same binding in the sense of `free-identifier=?`, and the match introduces no pattern variables.
