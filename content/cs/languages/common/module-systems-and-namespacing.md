...
Most C++ projects span multiple translation units that need to share declarations and definitions, and headers have been the mechanism for that since C. The mechanism is textual: [[cs/pl/macros-and-metaprogramming|the preprocessor]] pastes the header's contents into each unit before the compiler sees them. That gives no namespacing of its own (hence C's convention of prefixing every public symbol with a library-specific tag) and it means the standard library headers you include are recompiled in every translation unit that includes them.
