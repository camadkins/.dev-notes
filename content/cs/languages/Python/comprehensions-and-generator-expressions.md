...
Here is the rule verbatim in effect. Aside from the iterable expression in the leftmost `for` clause, the comprehension is executed in a separate implicitly nested scope. This ensures that names assigned to in the target list do not leak into the enclosing scope. So the loop variable of a comprehension is invisible outside it, unlike a plain `for` statement, whose target survives the loop.
