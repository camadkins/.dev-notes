...
- "Compiler," Wikipedia. https://en.wikipedia.org/wiki/Compiler . Supports the compilation model translating source code into a lower-level form ahead of time, the staged front-end/back-end phases (lexical analysis, parsing, semantic analysis, optimization, code generation), and its performance and early-error-detection advantages.
- "Interpreter (computing)," Wikipedia. https://en.wikipedia.org/wiki/Interpreter_%28computing%29 . Supports the interpretation model executing code directly at runtime, the trade-off of slower execution against immediate feedback and portability, and the existence of hybrid JIT systems that blend interpretation and compilation.
- "Abstract machine," Wikipedia. https://en.wikipedia.org/wiki/Abstract_machine . Supports abstract machines such as virtual machines and bytecode targets as intermediaries that separate language semantics from specific hardware.
