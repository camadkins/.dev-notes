...
- "Evaluation strategy," Wikipedia. https://en.wikipedia.org/wiki/Evaluation_strategy . Supports the distinctions among call-by-value, call-by-name, call-by-need, and call-by-reference, and how each determines when and whether function arguments are evaluated.
- "Lazy evaluation," Wikipedia. https://en.wikipedia.org/wiki/Lazy_evaluation . Supports call-by-need as lazy evaluation with sharing/memoization, its enabling of infinite data structures, and its interaction with side effects.
- "Short-circuit evaluation," Wikipedia. https://en.wikipedia.org/wiki/Short-circuit_evaluation . Supports the non-strict positions of Boolean and/or operators where the second operand is evaluated only when needed.