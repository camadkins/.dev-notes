...
![Bias falling and variance rising as model complexity grows, with total error forming a U shape and a sweet spot between underfitting and overfitting](assets/bias-variance-tradeoff.svg)
...
- [[generalization-vs-memorization]] is this tradeoff seen from the behavioral side
- [[train-validation-test]] is the measurement discipline that detects overfitting
- [[variance-and-covariance]] for the statistical machinery behind the variance term
- [[regression-fundamentals]] and [[simple-linear-regression]] for the squared-error setting where the decomposition is exact
- [[loss-functions]] on why training loss is a proxy, not the target
- [[gradient-descent]] can overfit simply by running too long, hence early stopping
