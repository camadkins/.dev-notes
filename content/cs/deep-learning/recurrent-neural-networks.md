...
Training treats the unrolled network as one deep computation graph, a technique called backpropagation through time (BPTT). Forward-pass a sequence, score the output sequence with a [[loss-functions|loss]] such as a weighted sum of cross-entropies, then run [[backpropagation]] backward through the unrolled graph, summing each weight's gradient contributions across all time steps before the [[gradient-descent]] update. The catch is that every time step adds depth, so a 100-step sequence behaves like a 100-layer network with tied weights.
...
> Sentiment analysis of a movie review is the classic sequence-to-vector setup: feed the review through the RNN one token at a time (each token converted to a vector by an [[embeddings|embedding]] layer), ignore the intermediate outputs, and read the final state as a summary of the whole review, which a last layer maps to a sentiment score.

Recurrence had a strong run in NLP, then attention mechanisms and the transformer architecture (attention without recurrence) took over the state of the art, partly because transformers train faster and suffer less from vanishing gradients. The gated-memory idea still matters: it is the cleanest illustration of how an architectural change, not more data, solved a fundamental optimization failure in the [[deep-learning-revolution|deep learning era]].

## Related Notes

- [[artificial-neural-networks|Artificial Neural Networks]], the feedforward foundation the recurrent cell extends
- [[backpropagation|Backpropagation]], the algorithm BPTT unrolls across time
- [[gradient-descent|Gradient Descent]], the update rule the summed gradients feed
- [[loss-functions|Loss Functions]], how an output sequence gets scored
- [[embeddings|Embeddings]], the learned word vectors an RNN consumes as input
- [[features-and-representations|Features and Representations]], the state vector as a learned summary
- [[deep-learning-revolution|The Deep Learning Revolution]], the era these architectures defined
- [[ai-vs-ml-vs-dl|AI vs ML vs DL]], where sequence models sit in the bigger picture
