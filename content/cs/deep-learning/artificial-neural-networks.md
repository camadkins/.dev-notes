...
![A perceptron unit: inputs times weights, summed, then passed through an activation to produce an output](assets/perceptron-unit.svg)
...
- [[backpropagation]] is how a multilayer network actually learns its weights
- [[activation-functions]] are the nonlinearities that give a stacked network its power
- [[gradient-descent]] is the optimizer that the training rule generalizes to
- [[loss-functions]] define the error the network is trained to reduce
- [[linear-algebra-fundamentals]] for weights and inputs as vectors and the weighted sum as a dot product
- [[deep-learning-revolution]] on how scaling these networks changed the field
- [[ai-vs-ml-vs-dl]] for where neural networks sit in the bigger picture
