...
> Everything above describes detection, and detection can be evaded. Attackers encode, split, and mutate payloads specifically to slip under whatever rules are active, and every raised paranoia level that closes an evasion opens a false positive somewhere else. A WAF is genuinely useful as defense in depth and as a fast shield while a real fix ships, but it sits in front of the vulnerability rather than removing it. The durable fix for [[sql-injection|SQL injection]] is parameterized queries in the application; the WAF is the outer layer that buys time and blunts the routine attacks, not a substitute for correct code.

## Related Notes

- [[firewalls|Firewalls]] - the layer-3/4 filter whose blind spot at the application layer a WAF fills
- [[sql-injection|SQL Injection]] - a primary attack class a WAF screens for, and one best fixed in code
- [[cross-site-scripting-xss|Cross-Site Scripting (XSS)]] - the other classic payload WAF rules target
- [[owasp-top-10|OWASP Top 10]] - the risk catalog whose injection and scripting entries WAF rules map to
- [[ids-and-ips|IDS and IPS]] - the network-layer analog of the same detect-versus-false-positive tradeoff
