...
NIST's glossary pins each term to a distinct act. Authentication is "verifying the identity of a user, process, or device, often as a prerequisite to allowing access to resources in an information system" (FIPS 200). It establishes a subject: a claimant proves possession of an authenticator, and the system now believes it knows who is making requests. Authorization is a different decision entirely, "the process of granting or denying specific requests for obtaining and using information and related information processing services" (NIST SP 800-53 Rev. 5), or put as a boundary decision, "the decision to permit or deny a subject access to system objects" (NIST SP 800-162).
