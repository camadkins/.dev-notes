...
- Alignment. "malloc and similar allocators tend to provide a degree of *alignment*, so that memory can only be provided to programs in chunks (usually a multiple of 4 bytes). As a result if a program requests perhaps 29 bytes, it will actually get a chunk of 32 bytes."
