...
![A magnetic-core memory plane. A core flips only where an X wire and a Y wire each carry half the needed current, so the pair selects exactly one bit.](cs/military-computing/assets/core-memory-plane.svg)
...
Magnetic-core memory was reliable and fast enough that it became [[cs/history/the-mosfet|the standard main memory of computers into the 1970s]], which is why older programmers still say "core" for memory. Whirlwind's design also carried forward directly. Its successor, Whirlwind II, became the basis for the [[cs/military-computing/sage-and-real-time-systems|SAGE]] air-defense system.

## Related Notes

- [[cs/military-computing/anfsq7-and-fault-tolerant-hardware|The AN/FSQ-7 and Fault-Tolerant Hardware]], the SAGE machine Whirlwind led to
- [[cs/military-computing/sage-and-real-time-systems|SAGE and Real-Time Systems]], the system built on this lineage
- [[cs/history/von-neumann-architecture|Von Neumann Architecture]], the stored-program model the memory serves
- [[cs/systems/virtual-memory|Virtual Memory]], a later layer over physical memory like this
