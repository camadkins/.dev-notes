...
That incomparability is exactly what [[topological-sorting|topological sort]] resolves: it embeds a partial order into a total order, producing one linear sequence consistent with every required precedence while inventing an order for the pairs the partial order left free. A build system, a course prerequisite chain, and a spreadsheet's recalculation order are all this move.
...
- [[disjoint-set|Disjoint Set]] - union-find maintains equivalence classes incrementally
- [[topological-sorting|Topological Sorting]] - linearizing a partial order into a consistent total order
