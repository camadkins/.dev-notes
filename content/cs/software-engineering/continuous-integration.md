...
- **A version-controlled mainline** with every commit built in a reference environment. Every push triggers a full build, and "only once this integration build is green can the developer consider the integration to be complete." Building on every push also localizes blame: a failure means "the fault lies in that latest push."
...
Wikipedia adds one requirement that is easy to overlook: CI "requires the version control system to support atomic commits," so a developer's changes land as a single unit rather than a partially-visible sequence.
