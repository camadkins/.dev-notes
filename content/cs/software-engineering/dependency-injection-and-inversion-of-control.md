...
The original sense is about control *flow*. Inversion of control is "a design principle in which custom-written portions of a computer program receive the flow of control from an external source (e.g. a framework)." In procedural programming "a program's custom code calls reusable libraries to take care of generic tasks, but with inversion of control, it is the external code or framework that is in control and calls the custom code." Fowler's own first encounter was exactly this: early user interfaces were driven by the application program, which issued a sequence of prompts, and then "with graphical (or even screen based) UIs the UI framework would contain this main loop and your program instead provided event handlers for the various fields on the screen." The slogan for it is the "Hollywood Principle: Don't call us, we'll call you." The etymology of the phrase "has been traced back to 1988."
