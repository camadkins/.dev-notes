...
- Quicksort, Wikipedia. https://en.wikipedia.org/wiki/Quicksort . Backs the divide-and-conquer partition-and-recurse structure and its in-place operation, the `O(n log n)` average and `O(n²)` worst-case entries in the infobox, the Lomuto scheme attributed to Nico Lomuto with pivot at the end and its degradation to `O(n²)` on already ordered input, the Hoare scheme as two pointers moving inward from both ends and exchanging inversions until they cross, and the specific claim that Lomuto "is inferior to Hoare's scheme because it does three times more swaps on average and degrades to `O(n²)` runtime when all elements are equal".
- Quicksort § Average-case analysis, Wikipedia. https://en.wikipedia.org/wiki/Quicksort#Average-case_analysis . Backs the randomization claim as this note states it: if the algorithm selects the pivot uniformly at random, the expected-time analysis "can be used to bound the expected running time for any input sequence", with the expectation taken over the algorithm's own random choices rather than over inputs.
- Quicksort § Space complexity, Wikipedia. https://en.wikipedia.org/wiki/Quicksort#Space_complexity . Backs the two stack claims: in-place partitioning needs `O(1)` space, and recursing into the smaller partition first and then handling the other by tail recursion or iteration keeps the stack depth at `O(log n)` even in the worst case, an idea credited to Sedgewick; without it the worst case makes `O(n)` nested calls and needs `O(n)` auxiliary space.
- Quicksort § Optimizations, Wikipedia. https://en.wikipedia.org/wiki/Quicksort#Optimizations . Backs the small-subarray cutoff to insertion sort as a standard Sedgewick optimization, with the caveat this note also makes: the ideal threshold varies by implementation and must be measured.
- Dutch national flag problem, Wikipedia. https://en.wikipedia.org/wiki/Dutch_national_flag_problem . Backs the attribution to Dijkstra, that the three-way partitioning pseudocode with indices maintaining `i ≤ j ≤ k` "was proposed by Dijkstra himself", and that quicksort variants needing robustness to repeated elements use it to group items less than, equal to, and greater than the pivot.
- Engineering a Sort Function, Jon L. Bentley and M. Douglas McIlroy, Software: Practice and Experience 23(11). https://cs.fit.edu/~pkc/classes/writing/samples/bentley93engineering.pdf . Backs the pivot-selection ladder: the paper's summary states it "chooses partitioning elements by a new sampling scheme" and "partitions by a novel solution to Dijkstra's Dutch National Flag problem", and the text names the ninther as Tukey's, "the median of the medians of three samples, each of three elements", adopted after median-of-three alone left the comparison count short of `n lg n`.
- Introsort, Wikipedia. https://en.wikipedia.org/wiki/Introsort . Backs the introspective fallback exactly as this note describes it: introsort begins with quicksort, switches to heapsort when the recursion depth exceeds a limit computed as `maxdepth ← ⌊log₂(length(A))⌋ × 2`, and switches to insertion sort below a size threshold, yielding `O(n log n)` worst-case runtime. It also backs the "used in many standard libraries" claim, naming libstdc++ (depth `2×log₂ n`, insertion sort under 16) and libc++.
- A Killer Adversary for Quicksort, M. D. McIlroy, Software: Practice and Experience. https://www.cs.dartmouth.edu/~doug/mdmspe.pdf . Backs the warning that structured input drives quicksort quadratic and that this is a real hazard rather than a theoretical one: production implementations have been caught going quadratic in real applications, and an adversarial comparison function can force quadratic behavior against any implementation, including a randomizing one, that meets mild assumptions.
- Compare (named requirement), cppreference.com. https://en.cppreference.com/w/cpp/named_req/Compare . Backs the comparator-consistency pitfall: a comparator must induce a strict weak ordering, specified as `comp(a, a) == false`, asymmetry, and transitivity, and standard sorting facilities assume it.
- Quickselect, Wikipedia. https://en.wikipedia.org/wiki/Quickselect . Backs the selection application: quickselect uses the same pivot-and-partition step but recurses into only the side holding the kth element, giving `O(n)` average time with an `O(n²)` worst case.
