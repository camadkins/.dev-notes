...
# - quicksort-partitions.svg — Side-by-side Lomuto vs Hoare partitions on A[l..r]; arrows show i/j scans and final pivot placement.

# - quicksort-recursion-tree.svg — Recursion tree for balanced vs worst-case splits; labels show work per level and depth.

# - three-way-partition.svg — <, =, > regions and pointers (lt, i, gt) for Dijkstra’s 3-way partition; ideal with many duplicates.

# - tail-recursion-elimination.svg — Schematic of always recursing on the smaller side and looping on the larger one to cap stack depth.
...
However, it can degrade to **O(n²)** when pivots are poor (e.g., already-sorted input with naive pivot choice) or when duplicates are handled naively. Practical implementations deploy **randomization**, **median-of-three**, **3-way partitioning**, and **introspective fallbacks** to avoid pathologies.
...
> [!example]  
> **Diagram (`quicksort-partitions.svg`)** — Animate pointers for Lomuto and Hoare on the same input to highlight swap counts and pivot placement differences.
...
- **Worst case**: highly unbalanced splits (e.g., pivot always min or max) → **depth ≈ n** → **O(n²)** time.
...
> [!example]  
> **Diagram (`tail-recursion-elimination.svg`)** — Show the pattern “recurse on smaller side, loop on larger side” and the resulting **O(log n)** stack depth line.
