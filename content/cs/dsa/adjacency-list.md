...
- Jeff Erickson, Algorithms, Chapter 5: Basic Graph Algorithms. https://jeffe.cs.illinois.edu/teaching/algorithms/book/05-graphs.pdf . Backs the definition of an adjacency list as an array of per-vertex neighbor lists, the storage rule that an undirected edge `uv` is stored twice while a directed edge is stored once in the tail's list, the overall `O(V + E)` space, listing a vertex's neighbors in `O(1 + deg(v))`, the edge test in `O(1 + deg(u))` by scanning one neighbor list, and the two bucket upgrades this note lists: `O(1 + log deg(u))` with a balanced binary search tree and constant time with a suitable hash table. It also backs the closing comparison, since it tabulates the matrix alternative at `Theta(V^2)` space, constant-time edge tests, and `Theta(V)` neighbor listing.
- Adjacency list, Wikipedia. https://en.wikipedia.org/wiki/Adjacency_list . Backs neighbor iteration costing constant time per neighbor so that the total is proportional to the degree of the vertex, the edge test on unsorted neighbors running in time proportional to the smaller of the two degrees by sequential search, and the sorted-array variant answering it by binary search in time proportional to the logarithm of the degree, which is the ordered-bucket row of the cost list.
- Graph (abstract data type), Wikipedia. https://en.wikipedia.org/wiki/Graph_%28abstract_data_type%29 . Backs the hash-bucket claims specifically: storing each vertex's adjacent set in a hash table gives amortized average constant time both to test adjacency of two given vertices and to remove an edge, which is what the `hasEdge` and `removeEdge` hash-set rows assert.
- std::unordered_set, cppreference.com. https://en.cppreference.com/w/cpp/container/unordered_set . Backs the `unordered_set<int>` bucket choice: search, insertion, and removal have average constant-time complexity, and the elements are organized into hash buckets rather than sorted, which is why this note warns the iteration order is implementation-dependent.
- std::set, cppreference.com. https://en.cppreference.com/w/cpp/container/set . Backs the `std::set<int>` bucket choice: it holds a sorted set of unique keys, so iteration is in ascending order, and search, removal, and insertion have logarithmic complexity, which is the `O(log deg)` figure quoted for tree-set buckets.
- std::vector, cppreference.com. https://en.cppreference.com/w/cpp/container/vector . Backs the `vector` bucket choice: elements are stored contiguously, insertion or removal at the end is amortized constant time (the basis for push-back inserts and swap-pop deletion), and the container documents which operations invalidate iterators, which is the hazard the iterator-invalidation pitfall names.
- Multigraph, Wikipedia. https://en.wikipedia.org/wiki/Multigraph . Backs the graph-policy invariant: a multigraph is precisely a graph permitted to carry multiple edges (parallel edges) between the same pair of vertices, and whether loops are also admitted is an authorial convention rather than a settled part of the definition, which is why the policy has to be fixed up front.
