...
aliases: []
# diagrams:
# - adjacency-examples.svg — One small graph shown as an edge set, adjacency list Adj[u], and adjacency matrix; highlights how each represents the same structure.
# - directed-vs-undirected.svg — Side-by-side mini graphs contrasting arrowed (directed) edges with undirected edges; shows in/out-degree vs degree.
---
...
> [!example]  
> **Diagram (`adjacency-examples.svg`)** — Show the _same_ 5-vertex graph as: (1) an edge set `{(u,v)}`, (2) an adjacency list `Adj[u]`, and (3) a `5×5` adjacency matrix. Label a couple of neighbor queries (e.g., “neighbors of 2?”) to connect notation to visuals.
...
- A **graph** is `G = (V, E)`, where `V` is a finite set of vertices (label them `0..n-1` for code) and `E ⊆ V × V` is the set of edges.
...
- Directed: **out-degree** `deg⁺(u) = |Adj[u]|`, **in-degree** `deg⁻(u) = |{ v : (v,u) ∈ E }|`.
    
- **Path**: sequence `u₀,u₁,…,u_k` with each consecutive pair an edge; **cycle**: path with `u₀ = u_k` and length ≥ 1 (directed cycles obey direction).
...
- **Adjacency matrix (A)**
    ...
    `A[u][v]=1` iff `(u,v)∈E`. Row sums give out-degrees; column sums give in-degrees.
...
- **Sparsity vs density.** Let `n=|V|`, `m=|E|`. Sparse graphs have `m ≪ n²` (e.g., `Θ(n)`); dense graphs approach `Θ(n²)`. Sparsity affects representation choice and algorithm costs.
    
- **Total degree rules.** Undirected: `∑_u deg(u) = 2m`. Directed: `∑_u deg⁺(u) = ∑_u deg⁻(u) = m`. These identities are handy for sanity checks.
...
- **Adjacency list (AL):** space `Θ(n+m)`, neighbor iteration is `Θ(deg(u))`. Best for sparse graphs and traversal-heavy work.
    
- **Adjacency matrix (AM):** space `Θ(n²)`, constant-time `hasEdge(u,v)`. Works well for dense graphs or matrix-style algorithms (e.g., transitive closure).
    

**Core operations (teaching-friendly costs).**

- Get neighbors of `u`: AL `Θ(deg(u))`; AM `Θ(n)` (scan row).
    
- Check `(u,v)` is an edge: AL `Θ(deg(u))` (or `O(1)` with a hash set per row); AM `Θ(1)`.
    
- Add/remove vertex: AL `Θ(1)`/`Θ(deg(u)+in(u))`; AM `Θ(n)` row/col work.
...
> [!example]  
> **Diagram (`directed-vs-undirected.svg`)** — Left: directed mini graph with arrows and labels `deg⁺(u)`/`deg⁻(u)`. Right: the same edges undirected, with a single `deg(u)` count per node.
