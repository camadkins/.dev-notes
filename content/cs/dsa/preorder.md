...
# - preorder-numbering.svg — A small binary tree labeled by visit order to illustrate Root→Left→Right sequencing.

# - iterative-stack-flow.svg — Stack push/pop timeline for the standard iterative preorder (push right first, then left).

# - morris-preorder.svg — Threaded edges and visitation points for O(1)-space Morris preorder.
...
- **process `u`**, then visit each `v ∈ Children(u)` in order.
...
> [!example]  
> **Diagram (`preorder-numbering.svg`)** — Number nodes by visit time to visualize Root→Left→Right.
...
- Time `Θ(n)`, auxiliary space `Θ(h)` where `h` is tree height.
    

> [!example]  
> **Diagram (`iterative-stack-flow.svg`)** — Show push order (right, then left) and resulting pop sequence to reinforce why it yields Root→Left→Right.
...
- **Time:** `Θ(n)` — each node is processed once.
    
- **Space:** `Θ(h)` for recursive and stack-based forms; **O(1)** for Morris (transient threading).
