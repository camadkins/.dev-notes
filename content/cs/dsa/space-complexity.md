...
aliases: []
# diagrams:
# - space-anatomy.svg — Partition total memory into input, output, auxiliary workspace, and recursion stack; highlight which parts count for S(n).
# - recursion-stack-vs-iteration.svg — Side-by-side trace of recursive vs iterative algorithm showing frame growth and reduction.
# - merge-vs-quick-space.svg — Compare merge sort’s Θ(n) buffer to quicksort’s Θ(log n) stack; annotate “in-place” nuances.
---
...
> [!example]
> **Diagram (`merge-vs-quick-space.svg`)** — Show merge’s array + buffer (two rows) versus quicksort’s single array with a shallow call stack at the side.
...
> [!example]
> **Diagram (`space-anatomy.svg`)** — Show memory stacks: input, output, working buffers, recursion stack. Highlight the **peak** where two buffers overlap.
