...
aliases: []
# diagrams:
# - euclid-modulo-steps.svg — Timeline of (a,b) → (b, a mod b) with decreasing remainders until zero; highlights the loop invariant gcd(a,b)=gcd(b,a mod b).
# - extended-euclid-bezout.svg — Back-substitution table showing x,y with ax+by=g where g=gcd(a,b); marks the column recurrences for coefficients.
---
...
> [!example]  
> **Diagram (`euclid-modulo-steps.svg`)** — Show a simple trace `(a,b)=(252,105)` stepping `(105,42) → (42,21) → (21,0)` with the invariant `gcd` unchanged each step.
...
> [!example]  
> **Diagram (`extended-euclid-bezout.svg`)** — A table like the one above with arrows indicating how `(old_x, x)` and `(old_y, y)` update each iteration; emphasize the invariant `old_x·a + old_y·b = old_r`.
