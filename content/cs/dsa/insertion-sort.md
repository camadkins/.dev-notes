...
# - insertion-gap-shift.svg — Show the growing sorted prefix A[0..i-1]; key=A[i] moves left by shifting larger elements right, then key is placed in the gap.

# - inversions-and-cost.svg — Small array with inversion pairs highlighted; count drops each pass as insertion sort fixes them, relating running time to number of inversions.
...
> [!example]  
> **Diagram (`insertion-gap-shift.svg`)** — Visualize the prefix `A[0..i-1]` shaded as sorted, the key `A[i]` in a buffer, and a rightward “wave” of shifts until a gap opens for the key.
...
> [!example]  
> **Diagram (`inversions-and-cost.svg`)** — Mark inversions in `[7,3,5,2,3]`: `(7,3)`, `(7,5)`, `(7,2)`, `(7,3)`, `(5,2)`, `(5,3)`. Each shift removes one inversion; the total shifts equals the number of inversions.
