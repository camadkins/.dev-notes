...
# - heapsort-build-and-sweep.svg — Stage 1: bottom-up heapify of A[0..n-1]. Stage 2: repeated (swap A[0],A[heap_end]; heap_end--; sift-down at 0) until heap_end=0.

# - heapsort-boundary-swap.svg — Off-by-one guard: show heap_end as exclusive size, contrasts “n vs heap_size” to avoid reading past end during the sweep.
...
> [!example]  
> **Diagram (`heapsort-build-and-sweep.svg`)** — Panel A: bottom-up heapify of `A`. Panel B: the “sweep”: swap `A[0]` with `A[heap_end]`, decrement `heap_end`, and `SIFT_DOWN(A, heap_end, 0)` until `heap_end == 0`.
...
> [!example]  
> **Diagram (`heapsort-boundary-swap.svg`)** — Show `heap_end` as an **exclusive** size and the shrinking active region `A[0..heap_end-1]`. This clarifies where to pass `n` to `SIFT_DOWN`.
