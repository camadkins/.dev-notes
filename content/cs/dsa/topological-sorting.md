...
aliases: []
# diagrams:
# - kahn-indegree.svg — Kahn’s algorithm: maintain indegrees, push zeros into a queue, pop and delete outgoing edges, revealing new zeros.
# - dfs-postorder.svg — DFS-based topo: run DFS, push node to a stack on exit; reverse exit order for the topological sequence.
# - dag-ambiguity.svg — A small DAG with multiple valid topo orders to illustrate non-uniqueness and tie-breaking.
---
...
> [!example]  
> **Diagram (`kahn-indegree.svg`)** — Show indegree counts on each node, a queue of zeros, and the process of popping a node, emitting it to the order, and decrementing its neighbors until the graph empties.
...
> [!example]  
> **Diagram (`dfs-postorder.svg`)** — Run DFS, label each vertex with discovery/finish times; arrows indicate edges; reverse the finishing order to display the topological sequence.
...
> [!example]  
> **Diagram (`dag-ambiguity.svg`)** — A DAG where nodes `{A, B}` are independent; both `[A, B, …]` and `[B, A, …]` are valid. Shows that topological order is not unique and why tie-breaking may be desirable.
