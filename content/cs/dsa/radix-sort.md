...
- Radix sort, Wikipedia. https://en.wikipedia.org/wiki/Radix_sort . Backs radix sort as a non-comparative algorithm that distributes elements into buckets by radix and repeats per digit "while preserving the ordering of the prior step", the `Θ(d(n + k))` running time with each digit in the range 0 to `k-1` "such as 0 to 255 for an 8-bit digit", that LSD sorts are generally stable while MSD sorts are not necessarily stable, the MSD recursion in which each bucket is sorted independently by the next digit with early termination once a bucket holds a single element, the variable-length contrast (LSD can group keys by length; MSD "must effectively extend all shorter keys to the size of the largest key"), the hybrid cutoff ("when the bins get small, other sorting algorithms should be used, such as insertion sort"), the signed-integer rule that "handling signed two's complement integers requires treating the most significant bit with the opposite sense, followed by unsigned treatment of the rest of the bits", and the parallelization split (the counting portion is data-independent and amenable to parallel reduce, while per-bin recursion is data-dependent).
- Lecture 10: Sorting III: Sorting Lower Bounds, Counting Sort, Radix Sort, MIT 6.006 Fall 2009. https://courses.csail.mit.edu/6.006/fall09/lecture_notes/lecture10.pdf . Backs the `d · (n + b)` figure in this note's overview and complexity section for n numbers in base b with d digits, the correctness argument for LSD ("more significant digits have stronger impact on the ordering than less significant digits, since they are processed at a later round"), the hard requirement that "digit sort needs to be stable, else will get wrong result", and the base trade-off this note frames as choosing k: for n numbers of b bits each, using r bits per digit costs `Θ((b/r)(n + 2^r))`, minimized when `r ≈ log n`.
- Sorting in linear time (CLRS 8.2, 8.3), Laura Toma, CS 231, Bowdoin College. https://tildesites.bowdoin.edu/~ltoma/teaching/cs231/2018spring/Lectures/linsort.pdf . Backs `O(d · (n + k))` when counting sort does each digit pass, the statement that "in order for this to work correctly, the intermediate sorting algorithm used must be stable", and the MSD trade-off this note repeats verbatim in spirit: MSD "generates many intermediate buckets" but "does fewer than d passes if inputs have a small distinguishing prefix". Its digit-width analysis (`log₂ x / r` digits each in range `[0, 2^r)`) backs choosing a power-of-two base so digit extraction is a shift and mask.
- Counting sort, Wikipedia. https://en.wikipedia.org/wiki/Counting_sort . Backs the per-pass subroutine: counting sort computes a histogram, applies a prefix sum to determine each key's position range, and places items in a final loop, with the relative order of equal keys preserved. It states directly that "because of its application to radix sorting, counting sort must be a stable sort", which is what makes an unstable pass a correctness bug rather than a performance one.
- Comparison sort, Wikipedia. https://en.wikipedia.org/wiki/Comparison_sort . Backs the claim that the `Ω(n log n)` floor is a property of the comparison model specifically: the bound follows from a decision tree needing at least `n!` leaves, so an algorithm that never compares two keys is outside its scope.
