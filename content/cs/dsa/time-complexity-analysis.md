...
aliases: []
# diagrams:
# - cost-model-and-growth.svg — Visual compare of step-count vs RAM operations and how both lead to asymptotic classes O(1), O(log n), O(n), O(n log n), O(n^2), etc.
# - recurrence-to-master.svg — Map common divide-and-conquer recurrences to Master Theorem cases with example points for merge sort and binary search.
---
...
> [!example]  
> **Diagram (`cost-model-and-growth.svg`)** can place these on the growth ladder: constant → logarithmic → linear → `n log n` → quadratic → cubic → exponential.
