...
aliases: []
# diagrams:
# - stack-vs-heap.svg — Side-by-side: call stack frames growing/shrinking automatically vs a process heap where blocks are requested/freed manually.
# - alloc-free-lifecycle.svg — Allocation timeline showing request → block returned → use → free; annotate double-free, use-after-free, and leak paths.
---
...
> [!example]  
> **Diagram (`stack-vs-heap.svg`)** — Show a function call adding a stack frame automatically, alongside a heap request that returns a pointer to a block which persists beyond the call unless explicitly freed.
...
> [!warning]  
> **Pitfalls illustrated in `alloc-free-lifecycle.svg`**
> 
