...
aliases: []
# diagrams:
# - dp-dag-and-table.svg — Subproblem DAG alongside a table; arrows show dependencies and a valid fill order (topological for DAG, row/column for table).
# - state-design-tradeoffs.svg — Side-by-side examples of “too-weak” vs “right” state definitions with their effects on transitions and complexity.
---
...
> [!example]  
> **Diagram (`dp-dag-and-table.svg`)** — Draw a subproblem DAG for edit distance `ED(i,j)` with edges to `ED(i-1,j)`, `ED(i,j-1)`, `ED(i-1,j-1)`. Next to it, show a 2D table; depict a valid fill order (row-major) that respects dependencies.
...
> [!example]  
> **Diagram (`state-design-tradeoffs.svg`)** — Compare a “too-weak” knapsack state `DP[w]` (no item index) that allows reusing items incorrectly vs the correct `DP[i][w]` (or order-optimized 1D with backward capacity loop). Show how state choice enforces constraints.
