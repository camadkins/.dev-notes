...
- Jessica Su, CS 161 Lecture 7 "Sorting in Linear Time", Stanford University (parts copied from CLRS). https://web.stanford.edu/class/archive/cs/cs161/cs161.1168/lecture7.pdf . Backs this note's trace element for element: the same input `A = [2, 5, 3, 0, 2, 3, 0, 3]` yields `Counts = [2, 0, 2, 3, 0, 1]` and cumulative sums `[2, 2, 4, 7, 7, 8]`, and the output `[0, 0, 2, 2, 3, 3, 3, 5]`. Also backs the `Θ(n + k)` runtime broken down across the four loops, the backwards iteration with a decrement of `CumSum[key]` as the mechanism that makes the sort stable, and the warning this note repeats: a single large key value such as `n²` is enough to destroy the linear runtime.
- Counting sort, Wikipedia. https://en.wikipedia.org/wiki/Counting_sort . Backs the `O(n + k)` time and space, that the running time is linear in the number of items and "the difference between the maximum key value and the minimum key value" (which is the `[a, b]` offset variant), the three-loop structure of histogram then prefix sum then reverse-order placement with the explicit note "the relative order of items with equal keys is preserved here; i.e., this is a stable sort", that counting sort is not a comparison sort so the `Ω(n log n)` lower bound does not apply, that it is often used as a subroutine in radix sort and "because of its application to radix sorting, counting sort must be a stable sort", the unstable in-place variant that reuses the input array with only the count array as auxiliary storage, and the parallelization by per-subarray count arrays that are merged.
- Lecture 10: Sorting III: Sorting Lower Bounds, Counting Sort, Radix Sort, MIT 6.006 Fall 2009. https://courses.csail.mit.edu/6.006/fall09/lecture_notes/lecture10.pdf . Backs the cumulative-frequency reading this note uses for the prefix sums: `C[j]` becomes the number of elements with value at most j, so elements of value j occupy positions `C[j-1] + 1` through `C[j]` of the output, and the four-loop version is annotated `Θ(n + k)`. Also backs that the digit sorter inside radix sort must be this stable counting sort or the result is wrong.
- Sorting in linear time (CLRS 8.2, 8.3), Laura Toma, CS 231, Bowdoin College. https://tildesites.bowdoin.edu/~ltoma/teaching/cs231/2018spring/Lectures/linsort.pdf . Backs `O(n + N)` time and `O(N + n)` extra space with the count-then-prefix-sum-then-place-backwards pseudocode, the flat statement "counting-sort is stable", and the multi-pass application that makes stability matter (sorting records by first name and then stably by last name).
