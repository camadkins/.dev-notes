...
aliases: []
# diagrams:
# - dary-heap-array-mapping.svg — 0-indexed array representation showing parent/child index formulas for arity d and a small example tree.
# - sift-down-dary.svg — One extract-min sift-down step: choose the minimum among d children, swap, and continue.
---
...
> [!example]  
> **Diagram (`dary-heap-array-mapping.svg`)** — Show a small min-heap with `d = 4`: the tree on the left and the array on the right. Label edges with child indices using `CHILD(i, k)` and highlight `PARENT(i)` mapping.
...
> [!example]  
> **Diagram (`sift-down-dary.svg`)** — One sift-down step highlighting the choice among `d` children and the swap with the smallest child; annotate indices.
