...
# - chaining-vs-open-addressing.svg — Side-by-side: (A) separate chaining with buckets of linked nodes; (B) open addressing array with probe sequence for a few inserts and a tombstone after a delete.

# - resize-rehash-flow.svg — Load factor crosses threshold → allocate new array (often ~2×), recompute indices, move entries; emphasizes newIndex = hash(k) mod newM.
...
> [!example]  
> **Diagram (`chaining-vs-open-addressing.svg`)** — Left: buckets as short lists under indices `h(k)`. Right: open addressing shows a cluster built by linear probing and a **tombstone** left by delete.
...
> [!example]  
> **Diagram (`resize-rehash-flow.svg`)** — Trigger at `α>LOAD_MAX` → allocate larger array → recompute `i = hash(k) mod newM` → move entries; show improved dispersion.
