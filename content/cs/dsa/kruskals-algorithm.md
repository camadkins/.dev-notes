...
# - kruskal-sorted-edges-dsu.svg — Edge list sorted by weight with a running forest; arrows show DSU find/union decisions (accept/reject) until |V|-1 edges are chosen.

# - dsu-path-compression-rank.svg — DSU trees before/after unions; illustrates path compression during FIND and union-by-rank/size to keep near-constant amortized cost.
...
> [!example]  
> **Diagram (`kruskal-sorted-edges-dsu.svg`)** — Show the sorted edge list with checkmarks on accepted edges and × on rejected (cycle-forming) ones, alongside a forest whose components merge via DSU unions.
...
> [!example]  
> **Diagram (`dsu-path-compression-rank.svg`)** — Show DSU trees flattening over time as `FIND` applies path compression, and unions attaching smaller ranks under larger to keep trees shallow.
...
- **Borůvka + Kruskal hybrids.** Run a few rounds of **Borůvka’s algorithm** to shrink components quickly (linear work), then finish with Kruskal on the reduced graph.
