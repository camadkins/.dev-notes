...
# - stack-transitions.svg — State machine of Empty → NonEmpty on push and NonEmpty → Empty on pop; shows top index changes.

# - array-vs-list-stack.svg — Side-by-side layouts: contiguous array with top pointer vs singly linked nodes with head as top.
...
function POP_ARRAY(S):
    ...
    S.n = S.n - 1
    x = S.A[S.n]
    // optional: S.A[S.n] = ⊥  // clear for GC/debug
...
> [!example]  
> **Diagram (`stack-transitions.svg`)** — Show transitions and how `n` and `top` move with each operation. Mark underflow on `POP` when `n==0` and resize on `PUSH` when `n==cap`.
...
- Array uses `Θ(cap)` elements; doubling/halving keeps wasted space ≤ 75% between resizes and ≤ 50% on average.
    
- List stores one pointer per element plus allocator overhead; total `Θ(n)` but larger constants.
...
- **Grow factor**: `×2` is standard; `×1.5` reduces spikes in large arrays at the cost of more resizes.
