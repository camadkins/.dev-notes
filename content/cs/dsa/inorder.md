...
- Tree traversal, Wikipedia. https://en.wikipedia.org/wiki/Tree_traversal . Backs the definition of in-order as LNR (traverse the left subtree, visit the node, traverse the right subtree), the guarantee that in a binary search tree in-order traversal retrieves the keys in ascending sorted order, the explicit-stack iterative form given here (push while descending left, then pop, visit, and move right), the statement that every such implementation costs stack space proportional to the height of the tree, and the existence of Morris traversal as the threading-based in-order implementation that removes that stack requirement.
- Threaded binary tree, Wikipedia. https://en.wikipedia.org/wiki/Threaded_binary_tree . Backs the threading mechanism this note's Morris section describes and the permanent variant mentioned under Optimizations: a binary tree is threaded by making right child pointers that would otherwise be null point to the in-order successor and null left pointers point to the in-order predecessor, which allows traversal without recursion and without the extra storage proportional to the tree's depth that recursion requires. It also backs the history, that Knuth posed the stackless in-order problem in a 1968 textbook and that Joseph M. Morris published this solution in 1979.
- Binary search tree, Wikipedia. https://en.wikipedia.org/wiki/Binary_search_tree . Backs the Core Idea and the first pitfall: the sorted output follows from the binary-search-tree property that every key in the left subtree is smaller and every key in the right subtree is larger, so an inorder tree walk visits all the nodes in order of non-decreasing key and nothing weaker guarantees that.
- CS 161 Lecture 8, Binary Search Trees, Jessica Su, Stanford University (some parts copied from CLRS). https://web.stanford.edu/class/archive/cs/cs161/cs161.1168/lecture8.pdf . Backs the `O(n)` running time claim, with the substitution-method proof that the inorder tree walk runs in `Θ(n)` because it visits all `n` nodes, and backs the successor and predecessor relationship the range and threading discussion leans on, defining the successor of a node as the smallest node greater than it and deriving the two cases directly from where the inorder walk prints next.
- Binary tree, Wikipedia. https://en.wikipedia.org/wiki/Binary_tree . Backs the two height figures quoted in the complexity discussion: a degenerate tree, where each parent has only one child, behaves like a linked list and so gives `h` on the order of `n`, while the minimum possible height for `n` nodes is `log2(n+1) − 1`, achieved by the balanced full or perfect shape.
- Order statistic tree, Wikipedia. https://en.wikipedia.org/wiki/Order_statistic_tree . Backs the k-th smallest optimization exactly as stated: store the subtree size at each node, maintain `size[x] = size[left[x]] + size[right[x]] + 1`, and implement Select by comparing `k` against `size[left]+1` and descending, which costs a single root-to-node path rather than a full traversal.
- C++ named requirements: Compare, cppreference. https://en.cppreference.com/w/cpp/named_req/Compare . Backs the comparator-consistency note, that the ordering a container relies on must be a strict weak ordering, and lists the properties such a relation is required to satisfy.
