...
# - heap-order-vs-shape.svg — Side-by-side: (A) a complete binary tree obeying heap-order, (B) a tree that obeys order but violates shape; captions clarify both properties are required.

# - array-index-mapping.svg — Annotated array A[0..n-1] with parent/child relations: left=2i+1, right=2i+2, parent=floor((i-1)/2); highlights last internal node floor(n/2)-1.
...
> [!example]  
> **Diagram (`heap-order-vs-shape.svg`)** — Panel A: a valid **max-heap** that is complete (shape ✓) and ordered (root ≥ children). Panel B: a tree that is ordered but **not complete** (a gap on the last level); explain that this **is not** a valid heap despite satisfying order locally.
...
> [!example]  
> **Diagram (`array-index-mapping.svg`)** — Show the array indices as nodes in a complete tree; draw arrows labeled `left=2i+1` and `right=2i+2`. Shade index `⌊n/2⌋−1` as the last internal node to emphasize where bottom-up heapify starts.
