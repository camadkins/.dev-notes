...
# - merge-sort-split-merge-tree.svg — Recursion tree: split array into halves down to size 1, then merge back up; labels show O(n log n) work.

# - stable-merge-two-pointers.svg — Two-pointer merge from L and R into temp; ties choose left-first to preserve stability.

# - bottom-up-iterative-passes.svg — Width=1,2,4,... passes over array; each pass merges adjacent runs of given width.

# - external-merge-kway.svg — External sorting pipeline: make runs that fit RAM, then k-way merge using a min-heap and replacement selection.
...
> [!example]  
> **Diagram (`stable-merge-two-pointers.svg`)** — Two arrows `i` and `j` walk the halves; equal elements from the left half are emitted first to preserve relative order.
