...
- Merge sort, Wikipedia. https://en.wikipedia.org/wiki/Merge_sort . Backs the `O(n log n)` bound in every case via the recurrence `T(n) = 2T(n/2) + n`, the stability claim and the `<=` tie-break that produces it (the reference implementation's merge takes from the left run when "left run head exists and is <= existing right run head"), the space split in the infobox (`O(n)` auxiliary for arrays, `O(1)` auxiliary with linked lists), the bottom-up formulation that merges runs of width 1, 2, 4, 8, 16 while ping-ponging between two buffers and copying a partnerless tail block, cache-aware tiled merge sort that stops partitioning at cache-sized subarrays and sorts each with insertion sort, and the parallel merge that partitions by binary searching splitter positions in each sorted subsequence.
- Merge sort § Natural merge sort, Wikipedia. https://en.wikipedia.org/wiki/Merge_sort#Natural_merge_sort . Backs run detection specifically: a natural merge sort exploits naturally occurring runs so it needs fewer passes, in the best case one pass over already sorted input, and it "is exploited as the key component of Timsort".
- Merge sort § In-place merge sort, Wikipedia. https://en.wikipedia.org/wiki/Merge_sort#In-place_merge_sort . Backs the "intricate, higher constants" characterization of `O(1)`-space stable merging: Geffert et al. showed in-place stable merging is possible in `O(n log n)` time with constant scratch space, but the algorithm is complicated and merging arrays of length n and m can take `5n + 12m + o(m)` moves; block merge sort is named as the modern stable, linear, in-place variant.
- Timsort, Wikipedia. https://en.wikipedia.org/wiki/Timsort . Backs Timsort as a hybrid stable sort derived from merge sort and insertion sort that finds already ordered runs and merges them, its use for non-primitive arrays in Java SE 7, Android, V8, and Swift, and the provenance of galloping (Carlsson, Levcopoulos and Petersson 1990; McIlroy 1993). Note that Python replaced Timsort with Powersort in 3.11, a derived natural mergesort with a different merge policy, so the natural-merge-plus-galloping lineage this note describes still holds for CPython.
- Timsort description (Objects/listsort.txt), Tim Peters, CPython. https://raw.githubusercontent.com/python/cpython/main/Objects/listsort.txt . Backs the galloping merge by its mechanism: after one run wins MIN_GALLOP consecutive comparisons the merge switches to galloping mode, searching exponentially ahead in the other run rather than comparing one element at a time. Also backs the small-run cutoff of 16 to 32 elements handled by binary insertion sort.
- External sorting, Wikipedia. https://en.wikipedia.org/wiki/External_sorting . Backs the run-generation and k-way merge structure: in the sorting phase, chunks small enough to fit main memory are read, sorted, and written to temporary files, then merged; and the reasoning for a single large k-way merge over repeated two-way passes, since each pass reads and writes every value from and to disk.
- K-way merge algorithm, Wikipedia. https://en.wikipedia.org/wiki/K-way_merge_algorithm . Backs the min-heap of run heads: maintain a min-heap keyed by each list's smallest current element, extract the root to the output, and reinsert from that list's tail, giving `O(n log k)`.
- Compare (named requirement), cppreference.com. https://en.cppreference.com/w/cpp/named_req/Compare . Backs the comparator pitfall: standard library sorts require a strict weak ordering, specified as `comp(a, a) == false`, asymmetry, and transitivity.
- Jessica Su, CS 161 Lecture 2, Stanford University (portions from CLRS). https://web.stanford.edu/class/archive/cs/cs161/cs161.1168/lecture2.pdf . Backs the merge subroutine's correctness argument, with the loop invariant that the filled output region holds the k-1 smallest elements of both inputs in sorted order, discharged at termination when the output holds all of them in order.
