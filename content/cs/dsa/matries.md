...
# - matrix-shapes-indexing.svg — A m×n grid annotated with 0-based indices (i,j); highlights row/column vectors, identity, and a transpose reflection across the main diagonal.

# - cache-friendly-loops.svg — Side-by-side loop nests for row-major vs column-major traversal; arrows indicate contiguous access vs cache-miss patterns.

# - sparse-formats.svg — COO triples (i,j,val), CSR row-pointer structure, and a tiny sparsity pattern heatmap contrasting dense vs sparse.
...
> [!example]  
> **Diagram (`matrix-shapes-indexing.svg`)** — Show a 2×3 grid with `(i,j)` labels, a highlighted main diagonal for square case, and arrows reflecting across the diagonal for transpose.
...
> [!example]  
> **Diagram (`sparse-formats.svg`)** — Tiny 5×6 matrix with 6 nonzeros shown as dots; next to it, show CSR arrays and how a row slices into `(col_idx, val)`.
