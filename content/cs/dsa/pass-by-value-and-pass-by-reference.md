...
# - call-vs-reference-passing.svg — Side-by-side stack frames for a function call: (A) value copy of parameters, (B) reference/alias to caller object; arrows indicate aliasing.

# - aliasing-hazards-checklist.svg — A small flow diagram that highlights: “Does callee keep a pointer/reference? Does it mutate? Does it outlive caller? → risks: UAF, data races, hidden coupling.”
...
> [!example]  
> **Diagram (`call-vs-reference-passing.svg`)** — Left: stack frame shows copies `a=1, b=2`. Right: arrows from callee parameters to caller variables; assignments update caller storage.
