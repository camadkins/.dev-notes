...
aliases: []
# diagrams:
# - bfs-dfs-trace.svg — Side-by-side trace of BFS and recursive DFS on the same small graph; shows discovery/finish times, tree edges, and BFS layers 0,1,2,…
# - time-stamps-and-edges.svg — DFS timeline with discovery d[u]/finish f[u] intervals that illustrate tree/forward/back/cross edge classification.
---
...
> [!example]  
> **Diagram (`bfs-dfs-trace.svg`)** — A five-vertex graph annotated with BFS **layers** (0,1,2) and a DFS **discovery/finish** timeline (`d[u]/f[u]`). Tree edges are bold; non-tree edges are dashed.
...
> [!example]  
> **Diagram (`time-stamps-and-edges.svg`)** — Horizontal bars for each vertex from `d[u]` to `f[u]`. Show that `(u,v)` is:
> 
> - **Tree/Forward:** intervals **nest** (`d[u] < d[v] < f[v] < f[u]`),
>     
> - **Back:** intervals **overlap** with `v` ancestor of `u` (`d[v] < d[u] < f[u] < f[v]`),
>     
> - **Cross:** intervals are **disjoint** (`f[v] < d[u]` or `f[u] < d[v]`) in directed graphs.
>     
...
- BFS: when `u` is dequeued, all vertices with `dist < dist[u]` are already processed; any neighbor `v` discovered later must satisfy `dist[v] ∈ {dist[u], dist[u]+1}` (depending on parallel edges/self-loops handling).
