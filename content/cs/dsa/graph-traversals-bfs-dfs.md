...
- **Cross edge:** to a vertex already finished when `u` was discovered (`f[v] < d[u]`). The reverse ordering `f[u] < d[v]` cannot occur for any edge `(u,v)`.
...
- Jeff Erickson, Algorithms, Chapter 6: Depth-First Search. https://jeffe.cs.illinois.edu/teaching/algorithms/book/06-dfs.pdf . Backs the timestamp machinery and the edge classification exactly as stated here. It defines the three vertex states as new, active, and finished in terms of the clock against `d[.]` and `f[.]`, shows that the active vertices are precisely the ones on the recursion stack and therefore always form a directed path (the GRAY-is-a-stack-path invariant), and gives the four edge classes by interval nesting: tree and forward edges satisfy `d[u] < d[v] < f[v] < f[u]`, back edges satisfy `d[v] < d[u] < f[u] < f[v]` and imply a directed path from `v` back to `u`, and cross edges satisfy `f[v] < d[u]`. It is also the source for the correction made to the cross-edge bullet, since it states that the fourth ordering, `f[u] < d[v]`, is impossible. Finally it backs the topological-sort claim, that every DAG has a topological ordering and the reversal of any postordering is one, and that a graph can be tested for acyclicity in `O(V + E)` from a postordering.
- Jeff Erickson, Algorithms, Chapter 5: Basic Graph Algorithms. https://jeffe.cs.illinois.edu/teaching/algorithms/book/05-graphs.pdf . Backs the framing that BFS and DFS are one traversal with two container choices, a queue giving breadth-first and a stack giving depth-first, and it supplies the precondition attached to every traversal cost in this cluster: the traversal is `O(V + E)` when the graph is in a standard adjacency list and rises to `O(V^2 + E)` when the same graph is stored as an adjacency matrix. It also backs the multiple-components pitfall, since the component-labeling wrapper that restarts the search at every unmarked vertex is what yields a spanning forest of the whole graph, still in `O(V + E)`.
- Depth-first search, Wikipedia. https://en.wikipedia.org/wiki/Depth-first_search . Backs the undirected-graph claim that DFS produces only tree edges and back edges, with no forward or cross edges, and backs reverse postordering producing a topological sorting of a directed acyclic graph. It also gives the `O(|V| + |E|)` time and `O(|V|)` space figures shared with BFS, and the iterative implementation that keeps a stack of neighbor iterators to reproduce the recursive traversal, which is the technique the iterative-DFS tip describes.
- Breadth-first search, Wikipedia. https://en.wikipedia.org/wiki/Breadth-first_search . Backs BFS exploring all nodes at the present depth before moving to the next, the queue holding the frontier, marking a vertex as explored before enqueueing it rather than at dequeue, the parent links tracing a shortest path back to the root, and the distance being measured in number of edges, which is the condition under which the layer numbers are shortest paths.
- Topological sorting, Wikipedia. https://en.wikipedia.org/wiki/Topological_sorting . Backs the two claims made in the topological-sort subsection: a topological ordering is possible if and only if the graph is a directed acyclic graph, and the DFS method is linear in the number of nodes plus edges. It also backs the point that the resulting order is not necessarily unique.
- Strongly connected component, Wikipedia. https://en.wikipedia.org/wiki/Strongly_connected_component . Backs the directed-reachability paragraph: the strongly connected components partition a directed graph, they can be found in linear time `Theta(V + E)`, and several algorithms based on depth-first search compute them, including Kosaraju's two-pass method and Tarjan's single-pass low-link method.
- Path-based strong component algorithm, Wikipedia. https://en.wikipedia.org/wiki/Path-based_strong_component_algorithm . Backs naming Gabow alongside Kosaraju and Tarjan as a DFS-based SCC method: it identifies Gabow's version (2000) as one of the path-based algorithms that run a depth-first search with two auxiliary stacks.
- Bipartite graph, Wikipedia. https://en.wikipedia.org/wiki/Bipartite_graph . Backs the bipartite test: a graph is bipartite exactly when it is 2-colorable and exactly when it contains no odd cycle, and the test can be run with breadth-first search giving each node the opposite color to its parent, where an edge to an already-colored vertex of the same color exhibits an odd cycle and proves the graph is not bipartite.
