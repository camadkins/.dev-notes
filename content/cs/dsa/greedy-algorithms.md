...
aliases: []    
# diagrams:
# - greedy-exchange-argument.svg — Visual proof idea: transform any optimal solution S* into the greedy solution G by a sequence of “exchanges” that never worsen cost.
# - interval-scheduling-counterexample.svg — Compare earliest-finish-time success vs a failing greedy rule (earliest start or longest duration) on the same intervals.
---
...
> [!example]  
> **Diagram (`greedy-exchange-argument.svg`)** — Show an optimal solution `S*` and the greedy solution `G`. At step `k`, illustrate exchanging `S*`’s `k`-th choice with `G`’s `k`-th without worsening cost, gradually morphing `S*` into `G`.
...
> [!example]  
> **Diagram (`interval-scheduling-counterexample.svg`)** — Same set of intervals shown against three rules: **earliest finish** (works), **earliest start** (fails), **shortest length** (fails). Mark the differing selections.
