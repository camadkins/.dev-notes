...
# - euclid-flow-and-trace.svg — Side-by-side: Euclid’s loop (a,b)→(b,a mod b) and a trace on (252,105) showing strictly decreasing remainders; last nonzero remainder equals gcd.

# - lcm-overflow-pitfall.svg — Visual of naive a*b overflow vs safe a/gcd(a,b)*b; annotate integer ranges and when to switch to big integers.
...
> [!example]  
> **Diagram (`euclid-flow-and-trace.svg`)** — Show the loop `(a,b) → (b, a mod b)` shrinking remainders until `b=0`. A trace for `(252,105)` ends with `gcd=21`.
...
> [!example]  
> **Diagram (`lcm-overflow-pitfall.svg`)** — Show `a=10^12`, `b=10^7`. Naive `a*b` overflows 64-bit; safe `a/g` first (`g=gcd(...)`) yields a product that fits, or directs you to big integers.
