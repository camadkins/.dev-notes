...
# - master-theorem-cases.svg — Case chart showing p=log_b(a) on the x-axis vs f(n) growth on the y-axis; highlights Case 1/2/3 regions and typical examples.

# - recursion-tree-levels.svg — Recursion tree for T(n)=aT(n/b)+f(n), annotating per-level cost a^i·f(n/b^i) and total via geometric/log factors.
...
**Recursion tree intuition** (use the `recursion-tree-levels.svg` diagram):
...
> [!example]  
> **Diagram (`master-theorem-cases.svg`)** — Show `p=log_b(a)` on the horizontal axis, `α` such that `f(n)≈n^α` on the vertical; Case 1 (α<p), Case 2 (α=p), Case 3 (α>p). Annotate common algorithms.
