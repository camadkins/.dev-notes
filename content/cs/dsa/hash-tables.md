...
aliases: []
# diagrams:
# - hash-table-chaining.svg — Array of buckets B[0..m-1] with keys distributed by h(k) and collisions stored in per-bucket lists; shows load factor and average chain length.
# - open-addressing-probes.svg — Probe sequences for linear/quadratic/double hashing on a small table; illustrates clustering, tombstones, and resize rehashing.
---
...
> [!example]  
> **Diagram (`hash-table-chaining.svg`)** — Show `m` buckets with linked lists or small arrays; define **load factor** `α = n/m` and illustrate average chain length around `α` under simple uniform hashing.
...
> [!example]  
> **Diagram (`open-addressing-probes.svg`)** — For the sequence above, draw arrows showing `(3)→(4)→(5)→(6)` under **linear probing**; contrast with **double hashing** where steps skip around the table, reducing primary clustering.
