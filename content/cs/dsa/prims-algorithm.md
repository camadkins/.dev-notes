...
# - prim-cut-property.svg — Visual cut (Tree, V\Tree); highlight the minimum crossing edge chosen at each step.

# - prim-adjlist-pq-workflow.svg — Dataflow: adjacency lists feed relaxations; PQ holds keys; parent[] records chosen edges.

# - prim-dense-o-n2.svg — Matrix-based Prim showing key[] scans across columns for dense graphs.

# - prim-vs-kruskal-density.svg — Heuristic chart comparing Prim (adj list + PQ) vs Kruskal (sort + DSU) across densities.
...
> [!example]  
> **Diagram (`prim-adjlist-pq-workflow.svg`)** — Show `Tree` on the left, non-tree vertices on the right. The minimum-key vertex pops from the PQ; its incident edges can lower neighbors’ keys; `parent[]` records chosen connections.
