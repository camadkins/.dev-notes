...
aliases: []
# diagrams:
# - string-layouts.svg — Contrast null-terminated (C-style) vs length-prefixed buffers with capacity fields and SSO; show bytes and metadata.
# - unicode-layers.svg — Code points vs code units vs grapheme clusters with examples (e.g., “é” as U+00E9 vs “e” + U+0301; emoji with skin-tone modifiers).
---
...
> [!example]
> **Diagram (`string-layouts.svg`)** — Show:
> - C-style: `[b0][b1]... [b(n-1)][0x00]`
> - Length-prefixed: `{len, cap} → [b0][b1]...[b(n-1)]`
> - SSO: header with inline bytes for short cases, fallback pointer for long.
...
> [!example]  
> **Diagram (`unicode-layers.svg`)** — Show the same human “character” represented as: (1) a single code point U+00E9, (2) `e` + COMBINING ACUTE U+0301, (3) emoji with ZWJ and skin tone. Emphasize that UI ops should iterate **graphemes**, not code units.
