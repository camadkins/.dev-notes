...
# - recurrence-tree.svg — Recursion tree for T(n)=aT(n/b)+f(n): fan-out, per-level work, and total across levels.

# - substitution-proof.svg — Induction template showing hypothesis, plug-in, simplify, and choose constants to bound T(n).

# - akra-bazzi-landscape.svg — Threshold graph comparing f(n) to n^p where p solves a·(1/b)^p=1; three regimes visualized.

# - dp-vs-dc-recurrence.svg — Side-by-side: cost recurrence for divide-and-conquer vs value recurrence for DP/memoization.
...
> [!example]  
> **Recursion tree (diagram: `recurrence-tree.svg`)**  
> For `T(n)=aT(n/b)+f(n)`, draw a tree with branching factor `a`. Each level `i` has `a^i` nodes of size `n/b^i`. The per-level cost is `a^i · f(n/b^i)`. Summing levels and bounding the leaf level yields `T(n)`.
...
> [!example]  
> **Diagram (`dp-vs-dc-recurrence.svg`)** — Left: `T(n)=2T(n/2)+n` (cost). Right: `OPT[i]=min(OPT[i−1]+w1, OPT[i−2]+w2)` (value). Same mathematics, different semantics and tooling.
