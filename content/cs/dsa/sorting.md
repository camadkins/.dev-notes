...
- Sorting algorithm, Wikipedia. https://en.wikipedia.org/wiki/Sorting_algorithm . Backs the stability definition (equal elements keep their input relative order), the comparison-table rows this note reproduces (insertion and bubble sort at best `n`, average and worst `n²`, `O(1)` memory, stable; selection sort `n²` in all three cases, `O(1)` memory, not stable, "makes exactly n-1 swaps"), the claim that practical implementations predominate as hybrids that cut over to insertion sort on small lists, and that Timsort is the standard sort in Java, Python, and Android while introsort is used in C++ and .NET.
- Comparison sort, Wikipedia. https://en.wikipedia.org/wiki/Comparison_sort . Backs the decision-tree lower bound: the tree must have at least `n!` leaves, so its height is at least `log₂(n!)`, and `log₂(n!) = Ω(n log n)`.
- Jessica Su, CS 161 Lecture 7 "Sorting in Linear Time", Stanford University (parts copied from CLRS). https://web.stanford.edu/class/archive/cs/cs161/cs161.1168/lecture7.pdf . Backs the derivation of `Ω(log(n!)) = Ω(n log n)` via Stirling's approximation, counting sort's `Θ(n + k)` bound, and the specific point that counting sort beats the comparison lower bound because it uses element values rather than comparisons.
- Lecture 10: Sorting III: Sorting Lower Bounds, Counting Sort, Radix Sort, MIT 6.006 Fall 2009. https://courses.csail.mit.edu/6.006/fall09/lecture_notes/lecture10.pdf . Backs the radix-sort running time `d·(n + b)` quoted in the table, the requirement that each digit pass be stable, and the classification of heapsort and selection sort as unstable.
- Bucket sort, Wikipedia. https://en.wikipedia.org/wiki/Bucket_sort . Backs the table's `Θ(n)` expected and `Θ(n²)` worst entries: total cost is `O(n + n²/k + k)`, so `k = Θ(n)` gives `O(n)` average on uniformly distributed input, while clustering everything into one bucket leaves the cost to the in-bucket sorter, `O(n²)` for insertion sort.
- Timsort description (Objects/listsort.txt), Tim Peters, CPython. https://raw.githubusercontent.com/python/cpython/main/Objects/listsort.txt . Backs the description of TimSort as an adaptive, stable, natural mergesort that identifies runs and merges them, and the insertion-sort cutoff sizes: minrun values of 16, 32, 64, and 128 tested about equally well, 256 was too costly and 8 too call-heavy, so CPython picks 32.
- std::sort, cppreference.com. https://en.cppreference.com/w/cpp/algorithm/sort . Backs "std::sort is unstable with n log n worst case": the specification says the order of equivalent elements is not guaranteed to be preserved and requires `O(N·log(N))` comparisons, and notes that this requirement is what rules out plain quicksort and is met by introsort.
- std::stable_sort, cppreference.com. https://en.cppreference.com/w/cpp/algorithm/stable_sort . Backs "std::stable_sort is stable and merge-based": the order of equivalent elements is guaranteed to be preserved and the function attempts to allocate a temporary buffer. Note the standard's bound is `O(N·log²(N))` comparisons, tightening to `O(N·log(N))` only when enough extra memory is available.
- Introsort, Wikipedia. https://en.wikipedia.org/wiki/Introsort . Backs the description of introsort as quicksort that switches to heapsort past a recursion-depth limit and to insertion sort below a size threshold, giving `O(n log n)` worst case, and that libstdc++ uses depth `2×log₂ n` with insertion sort under 16.
- Engineering a Sort Function, Jon L. Bentley and M. Douglas McIlroy, Software: Practice and Experience 23(11). https://cs.fit.edu/~pkc/classes/writing/samples/bentley93engineering.pdf . Backs attributing three-way partitioning for many duplicate keys to Bentley and McIlroy, and their statement that tripartite partitioning is equivalent to Dijkstra's Dutch National Flag problem.
- External sorting, Wikipedia. https://en.wikipedia.org/wiki/External_sorting . Backs the external merge sort procedure: chunks small enough to fit main memory are read, sorted, and written to temporary files, then combined with a k-way merge.
- Quickselect, Wikipedia. https://en.wikipedia.org/wiki/Quickselect . Backs quickselect as recursing into only one partition side, reducing average complexity from `O(n log n)` to `O(n)` with `O(n²)` worst case.
- Schwartzian transform, Wikipedia. https://en.wikipedia.org/wiki/Schwartzian_transform . Backs the decorate-sort-undecorate idiom as a way to avoid repeating expensive key extraction inside the comparator.
