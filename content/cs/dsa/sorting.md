...
# - sorting-landscape.svg — Map of algorithms by time bound (y-axis) vs memory usage (x-axis); highlight stability and in-place regions.

# - stability-demo.svg — Side-by-side example showing stable vs unstable behavior on records with equal keys.

# - comparison-lower-bound.svg — Decision-tree sketch for comparison sorts with Ω(n log n) leaf bound.
...
> [!example]  
> **Diagram (`sorting-landscape.svg`)** — Place common algorithms on a grid: y-axis time (`n`, `n log n`, `n²`); x-axis extra space (`O(1)` to `Θ(n)`). Use icons/marks to indicate **stable** vs **in-place**.
...
> [!example]  
> **Diagram (`comparison-lower-bound.svg`)** — Sketch decision tree with `n!` leaves and annotate `log(n!) ≈ n log n − Θ(n)`.
...
> [!example]  
> **Diagram (`stability-demo.svg`)** — Records `(grade, id)` with equal grades; show how a **stable** sort keeps equal-grade records in input order, while an **unstable** one may permute them.
