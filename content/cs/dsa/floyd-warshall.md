...
aliases: []
# diagrams:
# - fwk-iteration-layers.svg — 3D sketch of dist^k[i][j] where each layer k allows vertices {0..k} as intermediates; arrows show the min(dist, dist via k) relaxation.
# - fwk-path-reconstruction.svg — dist/next matrices for a 5-vertex example; highlight how next[i][j] updates when going through k and how to read back the actual path.
---
...
> [!example]  
> **Diagram (`fwk-iteration-layers.svg`)** — Visualize the DP as layers: each layer `k` allows routes that may use only vertices `{0..k}` as intermediates. For each `(i, j)`, update `dist[i][j] = min(dist[i][j], dist[i][k] + dist[k][j])`.
...
> [!example]  
> **Diagram (`fwk-path-reconstruction.svg`)** — Two small matrices for the example. Left: final `dist`. Right: `next` pointers; highlight the row for source `0` and show how following `next[0][3]`, `next[1][3]`, … spells out the path.
