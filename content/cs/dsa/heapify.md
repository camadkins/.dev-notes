...
# - heapify-levels.svg — Array-to-tree view labeling levels and indices; arrows show bottom-up sift-down from the last internal node to the root.

# - level-work-proof.svg — Bar chart of per-level work: n/2 nodes at height 0, n/4 at height 1, …; sum of nodes×height ≤ 2n gives O(n).
...
> [!example]  
> **Diagram (`heapify-levels.svg`)** — Show the array as a complete binary tree; highlight the last internal node and the sweep `i = ⌊n/2⌋−1 … 0`, each doing a sift-down to restore heap order.
...
> [!example]  
> **Diagram (`level-work-proof.svg`)** — Bars labeled by height: half the nodes at height 0, a quarter at height 1, …; area under the curve is ≤ `2n`, illustrating linear work.
