...
aliases: []
# diagrams:
# - ternary-interval-shrink.svg — On [l,r], place m1=l+(r-l)/3 and m2=r-(r-l)/3; compare f(m1) and f(m2), keep the better side for max (or opposite for min).
# - unimodal-discrete-trace.svg — Trace on a unimodal array A showing mid-neighbor comparisons and interval updates until the peak index is isolated.
---
...
> [!example]  
> **Diagram (`ternary-interval-shrink.svg`)** — Show `[l, r]` with `m1` and `m2`. If maximizing and `f(m1) < f(m2)`, shade left third as discarded; otherwise shade right third. Repeat, shrinking toward the peak.
...
> [!example]  
> **Diagram (`unimodal-discrete-trace.svg`)** — On a bitonic array, illustrate the mid vs mid+1 comparison and how the interval contracts toward the peak.
