...
- Tree (graph theory), Wikipedia. https://en.wikipedia.org/wiki/Tree_%28graph_theory%29 . Backs the definition of a tree as a connected acyclic undirected graph, the equivalent characterisation that any two vertices are joined by a unique simple path, the fact that a connected graph on `n` vertices has exactly `n − 1` edges, and the definition of a forest as a disjoint union of trees.
- Tree (abstract data type), Wikipedia. https://en.wikipedia.org/wiki/Tree_%28abstract_data_type%29 . Backs the rooted-tree vocabulary used here (root, parent, child, sibling, leaf, internal node, subtree, degree as a node's number of children), the height and depth conventions this note adopts (depth of the root is zero, height of a leaf is zero, height of an empty tree is `−1`), the definition of an ordered tree as one in which an ordering is specified for the children of each vertex, and the naming of preorder, inorder, postorder and level-order walks.
- Handshaking lemma, Wikipedia. https://en.wikipedia.org/wiki/Handshaking_lemma . Backs the degree sum formula, that the sum of all vertex degrees equals twice the number of edges, which combined with `|E| = n − 1` gives the `Σ_u degree(u) = 2(n − 1)` relation stated here.
- Binary tree, Wikipedia. https://en.wikipedia.org/wiki/Binary_tree . Backs the definitions of full (proper), complete, perfect and degenerate binary trees given in the Binary trees subsection; the perfect-tree counts `n = 2^{h+1} − 1` and `2^h` leaves; the leaf-versus-degree-2-node relation `L = I + 1`; the heap array layout with `left = 2i+1`, `right = 2i+2`, `parent = ⌊(i−1)/2⌋` for a zero-indexed root; and the claim that a preorder walk emitting null markers uniquely encodes a binary tree, which the article's EncodeSuccinct/DecodeSuccinct pair demonstrates by round-tripping.
- Left-child right-sibling binary tree, Wikipedia. https://en.wikipedia.org/wiki/Left-child_right-sibling_binary_tree . Backs the LCRS representation: two pointers per node, one to the first child and one to the next sibling, so that the children of a node form a singly linked list and reaching the k-th child requires walking that list.
- Lowest common ancestor, Wikipedia. https://en.wikipedia.org/wiki/Lowest_common_ancestor . Backs the naive parent-climbing LCA running in `O(h)`, and the Euler tour plus range-minimum-query construction that answers LCA queries in constant time after linear preprocessing.
- Binary heap, Wikipedia. https://en.wikipedia.org/wiki/Binary_heap . Backs the shape property that a binary heap is a complete binary tree, that this is what lets the array layout waste no space, and the `O(1)` find-min and `O(log n)` insert/delete costs quoted under Broader Implications.
- Red-black tree, Wikipedia. https://en.wikipedia.org/wiki/Red%E2%80%93black_tree . Backs the claim that balancing schemes bound tree height and therefore give `O(log n)` operations, and that this property is shared by AVL trees but not by ordinary binary search trees.
- Persistent data structure, Wikipedia. https://en.wikipedia.org/wiki/Persistent_data_structure . Backs the immutability point: persistent structures preserve every previous version, and path copying is the technique that makes cheap snapshots possible at the cost of extra memory.
- Succinct Trees in Practice, Arroyuelo, Canovas, Navarro and Sadakane (ALENEX 2010). https://users.dcc.uchile.cl/~gnavarro/algoritmos/ps/alenex10.pdf . Backs the succinct-representation tip: pointer form costs `O(n log n)` bits, while the balanced-parentheses (BP) and depth-first unary degree sequence (DFUDS) families use `2n + o(n)` bits, close to the information-theoretic bound, and still answer many tree operations in constant time.
- Treewidth, Wikipedia. https://en.wikipedia.org/wiki/Treewidth . Backs the Broader Implications claim that many problems which are NP-hard on general graphs are solved efficiently by dynamic programming once treewidth is bounded, and that tree decompositions are the machinery that extends tree algorithms to near-tree graphs.
- Tree traversal, Wikipedia. https://en.wikipedia.org/wiki/Tree_traversal . Backs the preorder/inorder/postorder/level-order orders listed under Traversals, that inorder retrieves the keys of a binary search tree in ascending order, and that level order needs space proportional to the maximum number of nodes at any one depth.
