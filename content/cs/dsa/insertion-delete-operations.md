...
# - percolate-up-down.svg — Two panels: (A) insert at end then sift-up to restore parent≥child (max-heap); (B) delete root, move last to root, then sift-down along larger-child path.

# - heap-index-boundaries.svg — Array mapping with indices; highlights root (0), last internal node floor(n/2)-1, and active range after extract.
...
> [!example]  
> **Diagram (`percolate-up-down.svg`)** — Panel A shows an append at the end index followed by parent swaps up to the root. Panel B shows removing the root: swap with last, shrink `n`, then repeatedly swap with the larger child until heap-order holds.
...
Leaves live at indices `⌊n/2⌋ .. n-1`. The **last internal node** is `⌊n/2⌋ − 1`.
...
> [!example]  
> **Diagram (`heap-index-boundaries.svg`)** — Mark `heap_end` as the exclusive bound during extracts. Shade `A[heap_end..n-1]` as the finalized suffix if using the sweep in [[cs/dsa/heapsort|Heapsort]].
...
- **Bounds & off-by-one.** Always guard child indices (`L < n`, `R < n`). The last internal node is `⌊n/2⌋−1`, not `⌊n/2⌋`.
