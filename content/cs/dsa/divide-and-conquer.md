...
aliases: []
# diagrams:
# - recursion-tree.svg — Recursion tree for T(n)=2T(n/2)+n with level costs and total summation; highlights logarithmic depth.
# - divide-combine-merge.svg — Visual pipeline showing split → recursive calls → combine step (e.g., merge) on array intervals A[l..m) and A[m..r).
---
...
> [!example]  
> **Diagram (`recursion-tree.svg`)** — Draw a recursion tree for `T(n)=2T(n/2)+n`. Annotate per-level work `n` and depth `log₂ n`, then sum to `n log n`.
...
> [!example]  
> **Diagram (`divide-combine-merge.svg`)** — Show `A[l..m)` and `A[m..r)` as two bars; arrows to two recursive calls; then a combine step (e.g., merge) writing back into `A[l..r)`.
