...
# - memory-layers.svg — Process view with static/data, stack (LIFO frames), heap (grows/shrinks), and OS pages; arrows show syscalls (mmap/sbrk) feeding user-space allocators.

# - alloc-strategies.svg — Visual compare of first-fit, best-fit, next-fit; highlight internal vs external fragmentation on a timeline of alloc/free events.

# - slab-size-classes.svg — Buckets for common object sizes (8,16,32,64…); per-class free lists and partial/empty/full slabs.

# - buddy-system.svg — Binary split/merge tree; show splitting 1MB into 512K→256K… and coalescing on free with buddy tags.

# - arena-bump.svg — Arena region with bump pointer advancing; instant reset/free of the whole region.

# - cache-locality.svg — Cache lines and objects; good vs poor spatial locality; false sharing example across cores.
...
> [!example]  
> **Diagram (`slab-size-classes.svg`)** — Show 64-KiB slabs carved into fixed 64-byte objects; “full/partial/empty” slab lists; pushes/pops from per-class free lists are `O(1)`.
...
> [!example]  
> **Diagram (`buddy-system.svg`)** — Split 1 MiB into 512 KiB → 256 KiB …; on free, walk back up if buddy is free.
