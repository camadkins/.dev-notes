...
# - linear-scan-early-exit.svg — Array A[0..n-1] with a moving index i; highlight comparisons and the moment search halts on a match or at i==n.

# - sentinel-trick.svg — Append sentinel x* at A[n]; loop drops the i<n check; after the loop, a single “i<n” test distinguishes found vs sentinel.
...
> [!example]  
> **Diagram (`linear-scan-early-exit.svg`)** — Show `i` moving across `A`; when `A[i]==x`, highlight the early exit and returned index.
