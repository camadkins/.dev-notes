...
# - sll-insert-delete-positions.svg — Small singly linked list showing insert/delete at head, after node X in the middle, and at tail; arrows highlight updated next pointers and (optional) tail changes.

# - traversal-early-exit.svg — Trace of a traversal with a predicate; shows early exit on a match, and how prev/curr pointers advance.
...
> [!example]  
> **Diagram (`sll-insert-delete-positions.svg`)** — Three panels: (A) insert at **head** (new node’s `next=head`, then `head=new`), (B) insert **after X** in the middle (rewire `new.next=X.next; X.next=new`), and (C) **delete tail** by scanning to the predecessor, then `pred.next=NIL` and update `tail` if tracked.
...
> [!example]  
> **Diagram (`traversal-early-exit.svg`)** — Show `prev/curr` marching along the list and halting when a predicate matches, emphasizing that only `curr` moves in a singly list unless you’re deleting the current node (which needs `prev`).
...
- **Space usage:** `Θ(n)` nodes; each node stores pointer overhead plus payload.
