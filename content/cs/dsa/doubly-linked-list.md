...
aliases: []
# diagrams:
# - dll-prev-next-wiring.svg — Three-node list showing head/tail, and correct prev/next wiring before and after a splice; highlights null sentinels vs dummy sentinel variant.
# - dll-insert-delete-local.svg — Stepwise local update around node X: insert-after and delete X with exact pointer rewires and order of operations.
---
...
> [!example]  
> **Diagram (`dll-prev-next-wiring.svg`)** — Show nodes `A ⇄ B ⇄ C` with `head=A`, `tail=C`. Annotate `prev`/`next` arrows and two variants: (1) explicit `null` at ends, (2) a circular **sentinel** node whose `prev`/`next` point to itself when empty. This clarifies boundary handling and simplifies algorithms.
...
> [!example]  
> **Diagram (`dll-insert-delete-local.svg`)** — Two panels: (1) **insert-after** `X` shows wiring order (`y.prev=X`, `y.next=X.next`, then fix neighbors). (2) **delete** `X` shows reassigning `X.prev.next` and `X.next.prev` **before** freeing `X`. Include end cases for null-terminated and the simplified sentinel version.
