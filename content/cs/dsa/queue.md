...
# - queue-ring-buffer.svg — Circular array with head/tail indices, wrap-around, and size; shows enqueue/dequeue movement.

# - queue-linked-list.svg — Head at front node, tail at last node with next = NIL; arrows for enq/deq.

# - producer-consumer-sequence.svg — Timeline of producers enqueuing and consumers dequeuing; highlights backpressure.
...
> [!example]  
> **Diagram (`queue-ring-buffer.svg`)** — Show a 6-slot array with `head=2`, `tail=5`, `n=3`: logical order is indices `2,3,4`. Enqueue writes at `5`; next `tail` becomes `0` by wrap-around.
...
> Producers call `TRY_ENQUEUE(x)`; if false (full), either drop, block, or backoff. Consumers call `TRY_DEQUEUE(&x)` in a loop; if false (empty), sleep or wait on a condition. See **`producer-consumer-sequence.svg`** for a timeline.
