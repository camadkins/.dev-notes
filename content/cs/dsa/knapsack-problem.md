...
Knapsack models **resource allocation**: budgets, cargo loading, ad placement under time caps, memory-limited caching, and even compiler optimizations (instruction selection). Understanding when **greedy** suffices and when **dynamic programming** is necessary is a core skill that bridges [[cs/dsa/greedy-algorithms|Greedy Algorithms]] and [[cs/dsa/dynamic-programming|Dynamic Programming]].
...
Use DFS over item order with an **upper bound** from **fractional** relaxation to prune. Effective when `n` is modest and structure exists; see [[cs/dsa/branch-and-bound|Branch and Bound]].
