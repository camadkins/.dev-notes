...
# - binary-heap-array.svg — Array layout of a binary heap with parent/child indices (0-based) and the heap-order property highlighted.

# - d-ary-heap-vs-binary.svg — Branching factor comparison (d=2,4,8) showing height reduction vs per-sift cost.

# - stable-priority-queue.svg — Tie-breaking with (priority, sequence) keys to achieve stability.
...
> [!example]  
> **Diagram (`binary-heap-array.svg`)** — Show the array cells and the parent/child mapping arrows to reinforce index arithmetic.
...
> [!example]  
> **Diagram (`d-ary-heap-vs-binary.svg`)** — Show height shrinking as `d` grows, with notes about increased comparison work per sift.
...
> [!example]  
> **Diagram (`stable-priority-queue.svg`)** — Illustrate composite ordering with equal priorities resolved by arrival order.
