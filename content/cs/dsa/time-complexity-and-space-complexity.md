...
aliases: []
# diagrams:
# - complexity-growth-ladder.svg — Visual ladder placing common classes O(1), O(log n), O(n), O(n log n), O(n^2), O(2^n) with example algorithms on each rung.
# - ram-vs-bit-vs-io.svg — Side-by-side depiction of RAM model (unit-cost ops), bit model (cost scales with operand length), and I/O model (block transfers).
# - memory-footprint-stack-heap.svg — Annotated layout showing algorithm data (inputs, outputs, auxiliary structures) to separate input storage from extra space.
---
...
> [!example]  
> **Diagram (`complexity-growth-ladder.svg`)** can place: constant → logarithmic → linear → `n log n` → quadratic → cubic → exponential, with examples like hash-table lookup, binary search, mergesort, all-pairs shortest paths, and brute-force subsets.
